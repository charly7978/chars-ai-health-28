...
export class RealTimeImageProcessor {
  ...
  public detectFingerPresence(imageData: ImageData, channels: ColorChannels): FingerDetection {
    ...
    return {
      ...
      edgeScore,
      colorConsistency,
      quality: confidence, // Usar confidence como quality
      area: coverage * width * height, // Calcular área basada en cobertura
      position
    };
  }
   ...
   */
  private estimateFingerPosition(redChannel: number[], width: number, height: number): { x: number; y: number; width: number; height: number } {
